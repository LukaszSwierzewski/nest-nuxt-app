...
export class UsersService {
  ...
  async create(createUserDto: CreateUserDto): Promise<User> {
    ...
    const afterLoginCookie = await this.sessionRepository.create({
      session_cookie: 'secret_random',
    });
  ...
  async findAllSession(userId: number): Promise<Session[]> {
  ...
  async findUserByCookie(session_cookie: string): Promise<any> {
    const oneUser = await this.sessionRepository.findOneOrFail(
      { session_cookie },
      {
        relations: ['session_user'],
      },
  ...
  async checkSessionExist(session_cookie: string): Promise<Session> {
    const foundSession = this.sessionRepository.findOneOrFail({
      session_cookie,
    });
    return foundSession;
  }
  async updateSessionID(id: number, session_cookie: any): Promise<void> {
